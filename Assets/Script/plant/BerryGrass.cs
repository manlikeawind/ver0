...
public class BerryGrass : BaseObj
{
    [System.Serializable]
    public class Attr
    {
    ...
    private void Awake()
    {
        ...
        berry = transform.Find("berry").gameObject;
        berryAnim = berry.GetComponent<Animator>();
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (windEnergy.x > 0.3 || windEnergy.x < -0.3 && isWind == false)
        {
            ...
            if (attr.berry == 1) {
                berryAnim.SetFloat("speed", anim_speed);
                berryAnim.SetBool("iswind", true);
            }
            isWind = true;
        }
        if (windEnergy.x < 0.3 && windEnergy.x > -0.3 && isWind == true)
        {
            ...
            if (attr.berry == 1)
            {
                berryAnim.SetBool("iswind", false);
            }
            isWind = false;
        }
